...
import { MessageSquare, Plus, Edit2, Trash2, Save, Settings2, Bell, Shield, Key } from 'lucide-react';
    ...
    <div className="container mx-auto p-6 max-w-6xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold flex items-center gap-2">
          ...
          Communications Hub Settings
        </h1>
        <p className="text-muted-foreground mt-2">
          Configure templates, notifications, and system preferences
        </p>
      </div>
...
function NotificationsSettings() {
  const { toast } = useToast();
  // TODO: Replace with actual API endpoint after cleanup
  const [settings, setSettings] = useState({
    businessPhone: '',
    handoffSmsEnabled: true,
    timeoutHours: 12,
    aiModel: 'gpt-4o-mini',
  });
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="businessPhone">Business Owner Phone (for SMS notifications)</Label>
            <Input
              id="businessPhone"
            ...
            <p className="text-xs text-muted-foreground mt-1">
              You'll receive SMS alerts when conversations need human attention
            </p>
          </div>
          
          <div className="flex items-center justify-between">
            <div>
              <Label>SMS Handoff Notifications</Label>
              <p className="text-sm text-muted-foreground">Get notified when AI hands off to human</p>
            </div>
            <Button variant="outline" size="sm">
              {settings.handoffSmsEnabled ? 'Enabled' : 'Disabled'}
            </Button>
          </div>
          
          <div>
            <Label htmlFor="timeoutHours">Auto-Return Timeout (hours)</Label>
            <Input
              id="timeoutHours"
              type="number"
              value={settings.timeoutHours}
              onChange={(e) => setSettings({ ...settings, timeoutHours: parseInt(e.target.value) })}
              min={1}
              max={48}
              className="mt-1"
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              After this time of inactivity, conversations return to AI mode
            </p>
          </div>
          
          <div>
            <Label htmlFor="aiModel">AI Model for Handoff Detection</Label>
            <Input
              id="aiModel"
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              Currently using GPT-4o-mini for cost-effective intent classification
            </p>
          </div>
          ...
          <div className="pt-4 border-t">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              ⚠️ Note: These settings are currently hardcoded. Full configuration coming in cleanup phase.
            </p>
          </div>
        </CardContent>
      </Card>
