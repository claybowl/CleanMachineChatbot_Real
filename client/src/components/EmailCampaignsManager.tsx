        ...
        <CardHeader className="pb-2">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div className="flex gap-2">
              ...
              <Button 
                className="bg-gradient-to-r from-purple-600 to-purple-500 text-white hover:from-purple-700 hover:to-purple-600 border-none shadow-sm"
              >
              ...
              <TableBody>
                {filteredCampaigns.length > 0 ? (
                  filteredCampaigns.map((campaign) => (
                    <TableRow key={campaign.id} className="hover:bg-blue-50 transition-colors">
                      <TableCell>
                        <div className="font-medium flex items-center">
                          {campaign.status === 'draft' && <span className="text-blue-500 mr-2">📝</span>}
                          {campaign.status === 'scheduled' && <span className="text-purple-500 mr-2">🗓️</span>}
                          {campaign.status === 'sent' && <span className="text-green-500 mr-2">✉️</span>}
                          {campaign.status === 'cancelled' && <span className="text-gray-400 mr-2">❌</span>}
                          {campaign.name}
                        </div>
                        <div className="text-sm text-gray-500 ml-6">{campaign.subject}</div>
                      </TableCell>
                      <TableCell>
