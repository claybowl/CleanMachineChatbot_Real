...
export async function handleGetAvailable(req: any, res: any) {
  try {
    ...
    // If calendar service is available, try to get actual availability
    if (calendarService) {
      try {
        ...
        console.error(
          "Error fetching from calendar, falling back to mock data:",
          calendarError,
        );
    ...
    // Generate mock slots (used as fallback or when calendar service is not available)
    const mockSlots = generateMockTimeSlots(serviceName);
...
/**
 * Generate mock time slots for appointments
 */
export function generateMockTimeSlots(serviceName: string) {
  const slots = [];
  const now = new Date();
  const duration =
  ...
  for (let day = 1; day <= 7; day++) {
    ...
    // Skip weekends unless it's an emergency request
    // Normal scheduling is only Monday-Friday (1-5)
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      continue; // Skip weekend days
    }

    // Generate slots during business hours (9am-3pm)
    // Start at BOOKING_HOURS.start (9am) and end at BOOKING_HOURS.end (15 = 3pm)
    for (let hour = BOOKING_HOURS.start; hour < BOOKING_HOURS.end; hour++) {
      // Skip 12pm slot (lunch break)
      if (hour === 12) continue;

      // Make sure service will be finished before closing time (5pm)
      if (hour + duration <= 17) {
        const slot = setHours(setMinutes(date, 0), hour);
        slots.push(slot.toISOString());
      }

      // Add half-hour slots for shorter services
      if (duration < 3 && hour + 0.5 + duration <= 17) {
        const halfHourSlot = setHours(setMinutes(date, 30), hour);
