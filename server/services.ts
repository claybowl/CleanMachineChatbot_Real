...
export async function saveServiceImage(serviceName: string, imageUrl: string): Promise<boolean> {
  try {
    ...
    await db.execute(
      sql`INSERT INTO services (name, price_range, description, duration, duration_hours, image_url, overview, detailed_description) 
          VALUES (${normalizedName}, 'See Google Sheets', 'See Google Sheets', '2 hours', 2, ${imageUrl}, 'See Google Sheets', 'See Google Sheets')
