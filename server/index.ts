...
// Configure PostgreSQL session store using Neon
const PgSession = connectPgSimple(session);
const pgPool = new NeonPool({
  connectionString: process.env.DATABASE_URL,
}) as any; // Type cast needed for connect-pg-simple compatibility

// Configure express-session with PostgreSQL store and secure HttpOnly cookies
app.use(session({
  store: new PgSession({
    pool: pgPool,
    tableName: 'session', // Will create 'session' table automatically
