...
export async function sendCampaignNow(id: number) {
  try {
    ...
    const recipients = await getRecipientsByAudience(campaign.targetAudience);
    ...
    if (customerList.length === 0) {
      try {
        ...
        customerList = sheetCustomers
          ...
          .map(customer => ({
            id: customer.id || String(Date.now() + Math.floor(Math.random() * 1000)),
...
async function getRecipientsByAudience(targetAudience: string) {
  ...
  switch (targetAudience) {
    case 'repeat_customers':
      ...
      query = query.where(sql`${customers.visitCount} > 1`);
      break;
    case 'new_customers':
      // New customers in the last 90 days
      const ninetyDaysAgo = new Date();
      ninetyDaysAgo.setDate(ninetyDaysAgo.getDate() - 90);
      query = query.where(gt(customers.createdAt, ninetyDaysAgo));
      break;
    case 'premium_customers':
      // Customers who have spent more than $500
      query = query.where(gt(customers.totalSpent, 500));
      break;
