    ...
    for (const conversation of timedOutConversations) {
      try {
        ...
        if (conversation.platform === 'sms' && customerNotification && conversation.customerPhone) {
          const timeoutMessage = `Hi${conversation.customerName ? ' ' + conversation.customerName : ''}! I noticed we haven't heard back in a while. Our AI assistant is back to help if you need anything! Or just text back if you'd like to continue with me directly.`;
