...
export async function sendWeatherAlertNotification(
  ...
  // Create the weather alert message
  const weatherAlertMessage = `
Hi ${name}, this is Clean Machine Auto Detail with a WEATHER ALERT for your upcoming appointment on ${formattedTime}.

Weather Forecast: ${severityText} for your scheduled appointment time.

${actionText}

To reschedule: Reply "RESCHEDULE" and we'll help you find a new time.
To keep your appointment: Reply "KEEP" and we'll see you as scheduled.

If you have questions, call us at (918) 555-0175
`;
...
This is Clean Machine Auto Detail with a WEATHER ALERT for your upcoming appointment on ${formattedTime}.
...
Options:
- Reply to this email with "RESCHEDULE" and we'll help you find a new time
- Reply with "KEEP" to maintain your current appointment time
...
`;
    ...
    try {
      ...
      emailResult = await sendEmail(email, subject, emailContent);
    } catch (error) {
